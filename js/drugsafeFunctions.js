...
function deleteAdminUser(idAdmin)
{
    jQuery('#loader').attr('style','display:block');
    $.post(__SITE_JS_PATH__+"admin/deleteAdminAlert",{idAdmin:idAdmin},function(result){
...
function deleteAdminConfirmation(idAdmin)
{
    $('.modal-backdrop').remove();
    $('#static').modal("hide");
    $('#clientStatus').modal("hide");
    jQuery('#loader').attr('style','display:block');
    $.post(__SITE_JS_PATH__+"admin/deleteAdminConfirmation",{idAdmin:idAdmin},function(result){
        var result_ary = result.split("||||");
        if(result_ary[0]=='SUCCESS')
        {
            $("#popup_box").html(result_ary[1]);
...
function hitPageUrl()
{
    var taxValue = $("#taxYear").val();
    var pageUrl = $("#pageUrl").val();
    
    window.location=__SITE_JS_PATH__+'admin/'+pageUrl+'/'+taxValue;
}
