...
function form_submit_validation(myForm, submit_form)
{
	...
	$('#' + myForm + '.registered-email').each(function(){
		validate_registered_email_field(this, submit_form)
	});
...
function in_line_validation()
{
	...
	$('.registered-email').blur(function(){
		validate_registered_email_field(this, true)
	});
...
function validate_registered_email_field(obj, is_auto_save)
{
	email = replaceAll($.trim($(obj).val()), ' ', '');
	email = replaceAll(email,',', '.');	
	email = removeAllTrailingDotsAndCommasInEmail(email);
	...
	{		
		...
		if(validateEmail(email))
		{
			$.ajax({
		    	type: "POST",
		      	url: __BASE_URL__ + "/admin/checkCustomerAlreadyExist",
		  		async: false,
		      	data: "id="+id+"&szEmail="+email+"&idUser=0",
		      	success: function(result){								
					if(result != '')
					{
						ar_result = result.split('||||');
